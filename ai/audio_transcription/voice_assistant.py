...
# from re import Match, DOTALL, match
...
#             return None
#
#     async def transcribe_chunk(self, chunk: AudioSegment) -> str | None:
#         # Save chunk to a temporary file-like object
#         chunk_io: BytesIO = BytesIO()
#         chunk.export(chunk_io, format="wav")
#         chunk_io.seek(0)
#
#         # Save it to a temporary path
#         temp_path: Path = Path("temp_chunk.wav")
#         with temp_path.open("wb") as f:
#             f.write(chunk_io.read())
#
#         # Transcribe the chunk
#         transcription: str | None = await self.trans_audio(temp_path)
#         logger.debug(f"Transcribed chunk: {transcription}")
#
#         return transcription
#
#     async def transcribe_all(self, chunks: list[AudioSegment]) -> str | None:
#         full_transcript_lines: list[str] = []
#         for chunk in chunks:
#             logger.info("Transcribing chunk...")
#             transcription: str | None = await self.transcribe_chunk(chunk)
#             if transcription:
